    ...
    Public Sub CargaOp()
        Try
            ...
            While Not EOF(Fnum)
                T = LineInput(Fnum)
                If T = "--sql_autos--" Then
                    provider_sql = "Provider=SQLOLEDB;Data Source=" & LineInput(Fnum) & ";Initial Catalog=ConAutomotriz;User ID=sa;Password=gotechdev;"
                End If
    ...
    Public Sub cargaListaCotiza(ByRef codigo As Integer, ByRef lista As ListView)
        ...
        If Rc.RecordCount > 0 Then
            ...
            Do While Rc.EOF = False
                ...
                t.SubItems.Add(Rc.Fields.Item("cot_cuota").Value)
                lista.Items.Add(t.Clone)
                Rc.MoveNext()
            Loop
            Rc.Close()
